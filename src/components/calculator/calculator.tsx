...
const Calculator = (): JSX.Element => {
    ...
    return(
        <div id="calculator">
            ...
            <div id="calc-buttons">
            {layout.map((row) => {
                return (<div className='button-row' key={row.toString()}>
                {row.map((value) => {
                    if (value === 'c') { return < ClearButton value={value} key={value} />}
                    if (value === '=') { return <EqualsButton value={value} key={value} />}
                    if (value === '-/+') { return <NegateButton value={value} key={value} />}
                    if (value === '%') { return<PercentageButton value={value} key={value} />}
                    if (['+','-','x','/'].includes(value)) { return <OperatorButton value={value} key={value} />}
